...
bool keyboard_lock[2][2]={0};//按鍵下次讀取到值是否可以直接執行[選擇按鍵][0:A 1:B]
bool Joystick_lock[2][2]={0};//搖桿下次讀取到值是否可以直接執行[選擇搖桿][0:y 1:x]
const int voltage_level[4]={3100,1000,1900,2100};//電位判斷0:高 1:低 2:無動作範圍(低) 3:無動作範圍(高)
byte player_connect[4]={0,0,0,0};//0:哪知搖桿先連接(0:無搖桿連接 1:搖桿0先連 2:搖桿1先連) 1:搖桿0是否被連接 2:搖桿1是否被連接 3:玩家數
int debouncetime=30;//除彈跳30毫秒
int Joystick_wait=300;//連續觸發300毫秒

////遊戲設定變數////
byte begin=0;//是否已選擇遊戲
byte win0=0,win1=0;//輸贏(0:未贏 1:贏 2:暫停選單選擇中回主選單)
bool menu_choose=0;//暫停選單的選項0:繼續 1:主選單
bool restart=0;//遊戲結束畫面是否重新開始
...
////遊戲暫停////
void game_stop_menu(void);//遊戲中選單
void game_end_menu(void);//遊戲中選單

////畫面管理////
//void Marquee_set(bool ,bool*);//設定跑馬燈文字
//void Marquee(byte,byte);//跑馬燈
void cutscenes(bool);//過場動畫
void paste(int/*y*/,int/*x*/,bool* /*輸入要貼的圖片*/,byte/*哪個地圖*/,byte/*陣列最大值y*/,byte/*陣列最大值x*/,int/*顏色*/,bool/*上下翻轉*/);//將圖片貼上
void paste(int/*y*/,int/*x*/,int* /*輸入要貼的圖片*/,byte/*哪個地圖*/,byte/*陣列最大值y*/,byte/*陣列最大值x*/,bool/*上下翻轉*/);//將圖片貼上(彩色)
...
void setup() 
{
  ...
  //資料處存初始化
  spiffs.init_SPIFFS();
  spiffs.file="/option.txt";//選擇設定檔案名字
  screen_brightness=spiffs.readFile(spiffs.file,1).toInt();//讀取亮度
  ws2812.setBrightness(screen_brightness);//限制亮度
  music.muted=spiffs.readFile(spiffs.file,2).toInt();//讀取聲音有無
...
void loop() 
{
  ...
  while(begin==0)//選單畫面
  {
    if(keyboard[0]==1||keyboard[1]==1||keyboard[2]==1||keyboard[3]==1||Joystick[0]!=0||Joystick[2]!=0)//任何搖桿有動作
    {
      ...
      if((Joystick[0]==2||Joystick[2]==2))//搖桿上
      {
        picture.menu_map[1]--;          
      }
      else if((Joystick[0]==1||Joystick[2]==1))//搖桿下搖
      {
        picture.menu_map[1]++;          
      }
        ...
        case 2://第二層選單
          switch(picture.menu_map[1])//遊戲選單地圖              
          {
            ...
            case 4://2048
              ...
              erase(0,0,1);//清除畫面
              paste(0,7,picture.ARROW[0],0,3,6,128,1);//貼上上箭頭
              paste(4,4,picture.m4[0],0,12,12,128,0);//貼上4 
              cutscenes(0);//過場動畫消失
              break;  
            case 5://沒有選單下
              picture.menu_map[1]--;
  ...
  if(begin==1)
  {
    ...
    if(picture.menu_map[1]==1)
    {
      ...
      Joystick_wait=150;//搖桿除彈跳設為150毫秒
        ...
        while(Tetris0.drop&&(Tetris1.drop||player_connect[3]==1)&&win0==0&&win1==0)
        {
          if(time_game.time_pass(0,1)>Tetris0.drop_time)//玩家0方塊落下時間到
          {
            ...
            if(Tetris0.state[2]==1||Tetris0.state[0]==1)//碰到方塊或超出下方邊界  
            {
              ...
              {
                if(Tetris0.drop_time_r>100)//下降速度高於100毫秒
                {
                  Tetris0.drop_time_r-=Tetris0.Combo*20;//加快下降速度(每列減少20毫秒)
                }
                else if(Tetris0.drop_time_r<100)//下降速度不低於100毫秒
                {
                  Tetris0.drop_time_r=100;
                }
                Tetris0.drop_time=Tetris0.drop_time_r;
              }
      ...
      while(win0==0&&win1==0)
      {
        ...
        else if(player_connect[3]==2)//雙人模式
        {
          if(time_game.time_pass(0,3)>snake0.move_time)//玩家0移動時間到
          {
            ...
            ////畫面更新
            erase(snake0.tail_location[0],snake0.tail_location[1],0);//清除玩家0蛇尾位置圖片
            if(snake0.head_location[0]>-1&&snake0.head_location[0]<20&&snake0.head_location[1]>-1&&snake0.head_location[1]<20)//避免遊戲結束時蛇頭亂跑
            {
              erase(snake0.head_location[0],snake0.head_location[1],0);//清除玩家0蛇頭位置圖片
              screen_out[0][1][snake0.head_location[0]][snake0.head_location[1]]=63;//貼上玩家0蛇頭位置圖片(青藍色)
              screen_out[0][2][snake0.head_location[0]][snake0.head_location[1]]=63;//貼上玩家0蛇頭位置圖片(青藍色)
            ...
            paste(0,0,snake0.Map[0],0,20,20,256*63,0);//貼上玩家0蛇身體位置圖片(綠色)
            erase(snake1.tail_location[0],snake1.tail_location[1],0);//清除玩家1蛇尾位置圖片
            if(snake1.head_location[0]>-1&&snake1.head_location[0]<20&&snake1.head_location[1]>-1&&snake1.head_location[1]<20)//避免遊戲結束時蛇頭亂跑
            {
              erase(snake1.head_location[0],snake1.head_location[1],0);//清除玩家1蛇頭位置圖片
              screen_out[0][0][snake1.head_location[0]][snake1.head_location[1]]=63;//貼上玩家1蛇頭位置圖片(橘色)
              screen_out[0][1][snake1.head_location[0]][snake1.head_location[1]]=32;//貼上玩家1蛇頭位置圖片(橘色)
            }
            screen_out[0][0][snake1.tail_location[0]][snake1.tail_location[1]]=63;//貼上玩家1蛇尾位置圖片(黃色)
    ...
    ////////2048////////
    else if(picture.menu_map[1]==4)
    {
      tm1637_0.showNumberDec(0,0);//顯示分數
      Joystick_wait=500;//搖桿除彈跳設為500毫秒
      if(player_connect[3]==1)//單人模式
      {
        g2048.contral=player_connect[0];//設定搖桿
        g2048.contral*=2;
        g2048.initialize();//初始化
        g2048.New();//新增兩個數字
        g2048.New();
        for(i=0;i<16;i++)//刷新整個螢幕畫面
        {
          if(g2048.number_log[i/4][i%4]==0)//沒數字
          {
            for(f=0;f<5;f++)//清除此格畫面
            {
              for(g=0;g<5;g++)
              {
                erase((i/4)*5+f,(i%4)*5+g,0);
              }
            }
          }
          else//有數字
          {
            for(f=0;f<5;f++)//放上底色
            {
              ...
              {
                erase((i/4)*5+f,(i%4)*5+g,0);
                screen_out[0][0][(i/4)*5+f][(i%4)*5+g]=g2048.back_color[0][g2048.number_log[i/4][i%4]%7];
                screen_out[0][1][(i/4)*5+f][(i%4)*5+g]=g2048.back_color[1][g2048.number_log[i/4][i%4]%7];
                screen_out[0][2][(i/4)*5+f][(i%4)*5+g]=g2048.back_color[2][g2048.number_log[i/4][i%4]%7];
              }
            }
            paste((i/4)*5,(i%4)*5,g2048.number[0][0],g2048.number_log[i/4][i%4]-1,5,5,255+256*255+256*256*255,0);//貼上數字圖片
          }
        }
        cutscenes(0);//過場動畫消失
        while(win0==0&&win1==0)//遊戲開始
        {
          if(Joystick[g2048.contral-1]!=0||Joystick[g2048.contral-2]!=0)
          {
            if(Joystick[g2048.contral-1]==1)//0玩家向右
            {
              g2048.direction=3;
            }
            else if(Joystick[g2048.contral-1]==2)//0玩家向左
            {
              g2048.direction=2;
            }
            if(Joystick[g2048.contral-2]==1)//0玩家向下
            {
              g2048.direction=1;
            }
            else if(Joystick[g2048.contral-2]==2)//0玩家向上
            {
              g2048.direction=0;
            }
            for(i=0;i<3;i++)//移動數字
            {
              g2048.move(g2048.direction);
              if(g2048.move_state==0)
              {
                break;
              }
              else
              {
                g2048.move_state_all=1;
              }
              for(f=1;f<6;f++)
              {
                erase(0,0,1);
                g2048.move_anime(g2048.direction,f);
                paste(0,0,g2048.number_color[0],0,20,20,0);//貼上還有移動的格子圖片(彩色)
                screen_change=1;//畫面改變
                delay(10);
              }
            }
            fraction[0][0]+=g2048.fraction_add;
            tm1637_0.showNumberDec(fraction[0][0],0);//顯示分數
            if(g2048.move_state_all==1)
            {
              g2048.New();
            }
            g2048.move_state_all=0;
            Joystick[g2048.contral-1]=0;
            Joystick[g2048.contral-2]=0;
            for(i=0;i<16;i++)//刷新整個螢幕畫面
            {
              if(g2048.number_log[i/4][i%4]==0)//沒數字
              {
            ...
            screen_change=1;//畫面改變
            if(g2048.amount==16&&g2048.juge()==1)
            {
              win1=1;//遊戲結束
              break;
            }
          }
          if(keyboard[g2048.contral-1])//0玩家B鍵被按下
          {
            keyboard[g2048.contral-1]=0;
            time_game.time_pass(2,0);
            for(i=3;i<5;i++)//計時暫停
            {
              time_game.time_pass(2,i);
  ...
  if(begin==2)//進入設定
  {
      ...
      while(1)
      {
        ...
        if(Joystick[1]!=0||Joystick[3]!=0)//搖桿有動作
        {
          if(f<16&&(Joystick[1]==1||Joystick[3]==1))//搖桿右
          {
            ...
            f++;
            screen_brightness+=2;
            spiffs.file="/option.txt";//設定檔案名字
            spiffs.message=String(screen_brightness);//設定紀錄資料
            spiffs.writeFile(spiffs.file,spiffs.message,1);//亮度寫入資料
            ws2812.setBrightness(screen_brightness);//限制亮度
          }
    ...
    /*else if(picture.menu_map[1]==8)//靈敏度設定
    {

    }*/
    else if(picture.menu_map[1]==8)//音量設定
    {
      ...
      while(1)
      {
        if(Joystick[0]!=0||Joystick[1]!=0||Joystick[2]!=0||Joystick[3]!=0)//搖桿y軸動作
        {
          ...
          spiffs.file="/option.txt";//設定檔案名字
          spiffs.message=String(music.muted);//設定紀錄資料
          spiffs.writeFile(spiffs.file,spiffs.message,2);//聲音有無寫入資料
          erase(0,0,1);//清除畫面
          if(music.muted==0)//目前為有聲
          {
            paste(4,4,picture.volume[0],0,12,12,128,0);//貼上聲音
...
void game_end_menu(void)//遊戲結束選單
{
  if(!(win0==2||win1==2))//暫停跳回主選單就不顯示結束選單
  {
    byte i,f,g,h;
    menu_choose=1;
    bool screen_state=0;//是否有顯示遊戲狀態
    input_zero();//搖桿設為未動作
    for(i=13;i<20;i++)//清除顯示選單區域畫面
    {
      for(f=1;f<19;f++)
      {
        erase(i,f,0);
    ...
    paste(13,1,picture.frame[0],0,9,18,256*256*63+256*63+63,0);//貼上方框圖片(白色)
    paste(15,10,picture.restart[0],0,5,6,256*63,0);//貼上重新開始圖片(綠色)
    paste(15,3,picture.back[0],0,5,6,63,0);//貼上返回選單圖片(紅色)
    for(i=0;i<13;i++)//備分會動區域畫面
    {
      for(f=0;f<20;f++)
      {
        for(g=0;g<3;g++)
    ...
    if(player_connect[3]==1)//單人模式結束遊戲
    {
      ...
      screen_state=1;//現在有顯示遊戲狀態畫面
      time_game.time_pass(1,4);//開始計時閃爍時間
    }
    else if(player_connect[3]==2)//雙人模式結束遊戲
    {
      if(win0==1&&win1==0)//玩家0贏畫面
      {
        for(i=0;i<13;i++)//清除顯示區域畫面
        {
          for(f=0;f<9;f++)
          {
            erase(i,f,0);
        ...
        paste(0,0,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
        paste(0,0,picture.win[0],0,15,9,63,0);//貼上win圖片(紅色)
        screen_state=1;//現在有顯示遊戲狀態畫面
        time_game.time_pass(1,4);//開始計時閃爍時間
      }
      else if(win0==0&&win1==1)//玩家1贏畫面
      {
        ...
        paste(0,11,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
        paste(0,11,picture.win[0],0,15,9,63,0);//貼上win圖片(紅色)
        screen_state=1;//現在有顯示遊戲狀態畫面
        time_game.time_pass(1,4);//開始計時閃爍時間
      }
      else if(win0==1&&win1==1)//平手畫面
      {
        ...
        paste(0,0,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
        paste(0,0,picture.tie[0],0,15,9,63,0);//貼上tie圖片(紅色)
        screen_state=1;//現在有顯示遊戲狀態畫面
        time_game.time_pass(1,4);//開始計時閃爍時間
      }
    }
    screen_change=1;//畫面改變
    while(!(win0==2||win1==2)/*返回主選選單*/)//選單
    {
      if(player_connect[3]==1)//單人模式結束遊戲
      {
        if(screen_state==0&&time_game.time_pass(0,4)>1000)//顯示遊戲狀態畫面
        {
          for(i=5;i<13;i++)//清除顯示區域畫面
          {
            for(f=1;f<19;f++)
            {
              erase(i,f,0);
        ...
        else if(screen_state==1&&time_game.time_pass(0,4)>1000)//消失遊戲狀態畫面
        {
          for(i=0;i<13;i++)//復原原本遊戲畫面
          {
            for(f=0;f<20;f++)
            {
              for(g=0;g<3;g++)
      ...
      else if(player_connect[3]==2)//雙人模式結束遊戲
      {
        if(win0==1&&win1==0)//玩家0贏畫面
        {
          if(screen_state==0&&time_game.time_pass(0,4)>1000)//顯示遊戲狀態畫面
          {
            ...
            paste(0,0,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
            paste(0,0,picture.win[0],0,13,9,63,0);//貼上win圖片(紅色)
            screen_change=1;//畫面改變
            screen_state=1;//現在有顯示遊戲狀態畫面
            time_game.time_pass(1,4);//開始計時閃爍時間
          }
        ...
        else if(win0==0&&win1==1)//玩家1贏畫面
        {
          if(screen_state==0&&time_game.time_pass(0,4)>1000)//顯示遊戲狀態畫面
          {
            ...
            paste(0,11,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
            paste(0,11,picture.win[0],0,13,9,63,0);//貼上win圖片(紅色)
            screen_change=1;//畫面改變
            screen_state=1;//現在有顯示遊戲狀態畫面
            time_game.time_pass(1,4);//開始計時閃爍時間
          }
        ...
        else if(win0==1&&win1==1)//平手畫面
        {
          if(screen_state==0&&time_game.time_pass(0,4)>1000)//顯示遊戲狀態畫面
          {
            ...
            paste(0,0,picture.end_frame[0],0,13,9,256*256*63+256*63+63,0);//貼上方框圖片(白色)
            paste(0,0,picture.tie[0],0,13,9,63,0);//貼上tie圖片(紅色)
            screen_change=1;//畫面改變
            screen_state=1;//現在有顯示遊戲狀態畫面
            time_game.time_pass(1,4);//開始計時閃爍時間
          }
      ...
      if(Joystick[1]!=0||Joystick[3]!=0)//搖桿有動作
      {
        ...
        {
          menu_choose=0;
          paste(15,10,picture.restart[0],0,5,6,63,0);//貼上重新開始圖片(紅色)
          paste(15,3,picture.back[0],0,5,6,256*63,0);//貼上返回選單圖片(綠色)
          screen_change=1;//畫面改變
        }
        else if(menu_choose==0&&(Joystick[1]==1||Joystick[3]==1))//搖桿向右
        {
    ...
    if(fraction[0][0]>fraction[0][1])//最高分紀錄
    {
      fraction[0][1]=fraction[0][0];
    }
    if(fraction[1][0]>fraction[1][1])//最高分紀錄
    {
  ...
  for(;;)
  {
    ...
    for(g0=0;g0<6;g0++)
    {
      ...
      if(player_connect[1])//搖桿0連接
      {
        for(i0=0;i0<2;i0++)
        {
          Joystick_log[0][i0][g0]=analogRead(keyboard_pin[0][i0]);//紀錄搖桿數y、x值
          //Serial.println(Joystick_log[0][i0][g0]);
          if(Joystick_log[0][i0][0]>voltage_level[2]&&Joystick_log[0][i0][0]<voltage_level[3]&&Joystick_log[0][i0][1]>voltage_level[2]&&Joystick_log[0][i0][1]<voltage_level[3]&&Joystick_log[0][i0][2]>voltage_level[2]&&Joystick_log[0][i0][2]<voltage_level[3]&&Joystick_log[0][i0][3]>voltage_level[2]&&Joystick_log[0][i0][3]<voltage_level[3]&&Joystick_log[0][i0][4]>voltage_level[2]&&Joystick_log[0][i0][4]<voltage_level[3]&&Joystick_log[0][i0][5]>voltage_level[2]&&Joystick_log[0][i0][5]<voltage_level[3])//是否解鎖
          {
            Joystick_lock[0][i0]=1;
          }
          if(time_contral.time_pass(0,i0)>Joystick_wait||Joystick_lock[0][i0])//判斷搖桿y、x狀態
          {
            if(noise_removal(i0)>voltage_level[0])//判斷為下或右
            {
              Joystick_lock[0][i0]=0;
              time_contral.time_pass(1,i0);
              Joystick[i0]=1;
            }
          ...
          if(keyboard_lock[0][i0]==0&&time_contral.time_pass(0,i0+2)>debouncetime&&digitalRead(keyboard_pin[0][2+i0])==0)//是否解鎖
          {
            keyboard_lock[0][i0]=1;
            time_contral.time_pass(1,i0+2);//重新計時除彈跳時間
          }
          if(keyboard_lock[0][i0]&&time_contral.time_pass(0,i0+2)>debouncetime&&digitalRead(keyboard_pin[0][2+i0]))//(除彈跳時間過或是解鎖)且讀值為按下
          {
            time_contral.time_pass(1,i0+2);//重新計時除彈跳時間
            keyboard_lock[0][i0]=0;//上鎖
            keyboard[i0]=1;//按鍵設為按下
          }
        } 
      }
      if(player_connect[2])//搖桿1連接
      {
        ...
        {
          Joystick_log[1][i0][g0]=analogRead(keyboard_pin[1][i0]);//紀錄搖桿數y、x值    
          if(Joystick_log[1][i0][0]>voltage_level[2]&&Joystick_log[1][i0][0]<voltage_level[3]&&Joystick_log[1][i0][1]>voltage_level[2]&&Joystick_log[1][i0][1]<voltage_level[3]&&Joystick_log[1][i0][2]>voltage_level[2]&&Joystick_log[1][i0][2]<voltage_level[3]&&Joystick_log[1][i0][3]>voltage_level[2]&&Joystick_log[1][i0][3]<voltage_level[3]&&Joystick_log[1][i0][4]>voltage_level[2]&&Joystick_log[1][i0][4]<voltage_level[3]&&Joystick_log[1][i0][5]>voltage_level[2]&&Joystick_log[1][i0][5]<voltage_level[3])//是否解鎖
          {
            Joystick_lock[1][i0]=1;
          }
          if(time_contral.time_pass(0,i0+4)>Joystick_wait||Joystick_lock[1][i0])//判斷搖桿y、x狀態
          {
            if(noise_removal(i0+2)>voltage_level[0])//判斷為下或右
            {
              Joystick_lock[1][i0]=0;
          ...
          if(keyboard_lock[1][i0]==0&&time_contral.time_pass(0,i0+6)>debouncetime&&digitalRead(keyboard_pin[1][2+i0])==0)//是否解鎖
          {
            keyboard_lock[1][i0]=1;
            time_contral.time_pass(1,i0+6);//重新計時除彈跳時間
          }
          if(keyboard_lock[1][i0]&&time_contral.time_pass(0,i0+6)>debouncetime&&digitalRead(keyboard_pin[1][2+i0]))//(除彈跳時間過或是解鎖)且讀值為按下
          {
            keyboard_lock[1][i0]=0;//上鎖
            time_contral.time_pass(1,i0+6);//重新計時除彈跳時間
            keyboard[2+i0]=1;//按鍵設為按下
          }     
        } 
      }
    }   
...
void paste(int y,int x,bool *MAP,byte MAP_Z,byte MAP_max_y,byte MAP_max_x,int color,bool flip)
  ...
  int color_r=color;
    for(i=0;i<MAP_max_y;i++)
    {
      for(f=0;f<MAP_max_x;f++)
      {
        ...
        {
          if(*(MAP+(MAP_Z*MAP_max_y*MAP_max_x)+(i*MAP_max_x)+f))//有點的地方再貼上
          {
            for(g=0;g<3;g++)
            {
              if(flip)
...
void paste(int y,int x,int *MAP,byte MAP_Z,byte MAP_max_y,byte MAP_max_x,bool flip)
  ...
  int color=0;
    for(i=0;i<MAP_max_y;i++)
    {
      for(f=0;f<MAP_max_x;f++)
      {
        if(((y+i)<20)&&((y+i)>-1)&&((x+f)<20)&&((x+f)>-1))//在螢幕裡的點
        {
          if(*(MAP+(MAP_Z*MAP_max_y*MAP_max_x)+(i*MAP_max_x)+f)>0)//有顏色的地方再貼上
          {
